...
if pass=pass1 then
 ...
 if rs.eof then                                                    '如果到最后还没有和输入的密码相同的则转到err.html
     ...
     response.write "<script>alert('学号或工号已注册');location='index.asp';onclick=history.go(-1)</script>"
    response.end                                                           '并且结束整个过程
end if                                                                 '结束if 语句
 rs.close                                                          '结束一切rs操作
 set rs=nothing                                                     '清空rs
