...
<div id="Layer6">
  <p>&nbsp;</p>
  <div id="Layer12">
    <div id="Layer14">当前位置：<a href="synr.asp">实验室</a>&gt;&gt;实验室借用时间安排表</div>
...
<div id="Layer8">
  <p class="STYLE4">请指导老师注意在学生借好钥匙并归还之后点击【完成】按钮</p>
  <p>
  ...
  ok="1"
       ...
       sql="select * from content where ok>'"&ok&"' and ok1<>'"&ok&"' order by id DESC  "
...
response.write "<BR> <p align='center' class='STYLE3'>暂时没有需处理的借用实验室申请" & allrows & "</p>"
response.end
  else
...
%>
  ...
  <table border="0" cellpadding="3" cellspacing="1" width="776" align="center" ;">
    <tr> 
      <td width="156"  >

         <span class="STYLE6">
           <a class="STYLE2">
             <img src="../image/d1.gif" width="10" height="10" border="0">实验室：<%=rs("name1")%>         </a>        </span>      </td>
	  <td width="191">申请人：<%=rs("nam1")%></td>
      <td width="246"><span class="STYLE2">申请使用时间：<%=rs("time1")%></span>	        </td>
	  <%if session("limit")>"1"then %>
	 
      <td width="154"><div align="left"><a href="did.asp?id=<%=rs("id")%>">完成</a></div></td>
      <% else %>
      <% end if %>     
    </tr>
  </table>
...
%>
      ...
      <%
if page=>8 then
 Response.Write"<a href=sqyl.asp?page=1>第一页</a>"
...
%>
    ...
    %>
      ...
      </strong><a href="sqyl.asp?page=<%=page+1%>">下一页</a>
      <%else%>
